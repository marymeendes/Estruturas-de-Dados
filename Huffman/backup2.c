...
int ehFolha(ArvCod* t){
	return t->esq == NULL && t->dir == NULL;
}

int decodifica(FILE* compacto, ArvCod* arvDecod, FILE* descompacto){
	char c;

	if(fread(&c, sizeof(char), 1, compacto)){
		if(!ehFolha(arvDecod)){
			if(c == '0'){
				decodifica(compacto, arvDecod->esq, descompacto);
			}else if(c == '1'){
				decodifica(compacto, arvDecod->dir, descompacto);
			}
		}else{
			fwrite(&arvDecod->info->caracter, sizeof(char), 1, descompacto);
			return 1;
		}
	}else{
		return 0;
...
void decodificacao(FILE* compacto, ArvCod* arvDecod, FILE* descompacto){
	rewind(compacto);
	while(decodifica(compacto, arvDecod, descompacto)){}
}
