/*
	Trabalho 2 de ED2
	Algoritmo: Implementação do método de Seleção com Substituição para geração de partições classificadas de arquivos muito grandes.
	Autor: Mariana Mendes da Silva
...
void substituicao (char* nome, int memorySize);
...
int main(void){
	...
	substituicao("arquivo.dat", 6);

	return 0;
}

void substituicao (char* nome, int memorySize){ //algoritmo de seleção por substituição que recebe um arquivo de inteiros e o tamanho da memória disponível
	...
	printf("Sucesso!\n%d particoes criadas!\nPara vê-las execute o programa leitura.c :3\n", indice-1);
